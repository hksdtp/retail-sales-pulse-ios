// Script để cấu hình tự động Google Sheets

// Thông tin cấu hình
const config = {
  sheetId: '18suuPRMUScXDWPmvIzmbZpLOTlEViis5olm7G1TqV1A',
  apiKey: 'AIzaSyAXwhLirER9EBC101q9mhyT5Uyj06bC5rg',
  serviceAccount: {
...
localStorage.setItem('googleSheetsApiKey', config.apiKey);
localStorage.setItem('googleServiceAccount', JSON.stringify(config.serviceAccount));

console.log('Đã cấu hình Google Sheets thành công với:');
console.log('- Sheet ID:', config.sheetId);
console.log('- API Key:', 'đã được cấu hình');
console.log('- Service Account Email:', config.serviceAccount.client_email);

// Reload trang để áp dụng cấu hình
