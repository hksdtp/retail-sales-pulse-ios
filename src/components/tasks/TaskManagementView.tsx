...
import { getTypeName, getStatusColor } from './task-utils/TaskFormatters';
...
export default function TaskManagementView({
  ...
  const isManager = currentUser?.role === 'team_leader' || currentUser?.role === 'retail_director' || currentUser?.role === 'project_director';
    ...
    switch (view) {
      case 'personal':
        ...
        return regularTasks.filter(task =>
          task.assignedTo === currentUser?.id || task.user_id === currentUser?.id
        );
      case 'team':
        if (isManager) {
          ...
          return regularTasks.filter(task =>
            task.isSharedWithTeam && task.team_id === currentUser?.team_id
          );
        }
      case 'individual':
        if (isManager) {
      ...
      default:
        return regularTasks.filter(task => task.assignedTo === currentUser?.id || task.user_id === currentUser?.id);
    }
  };
  ...
  const getTaskViewButtons = () => {
    const baseButtons = [
      { icon: User, label: 'Của tôi', value: 'personal' }
    ];
    ...
    if (isManager) {
      ...
      baseButtons.push(
        { icon: Users, label: 'Của nhóm', value: 'team' }
      );
    }

    // Thêm công việc chung cho tất cả
    baseButtons.push(
      { icon: Globe, label: 'Chung', value: 'department' }
    );
  ...
  const handlePriorityChange = async (e: React.MouseEvent, taskId: string, currentPriority: string) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    try {
      console.log(`🎯 Chuyển mức độ ưu tiên task ${taskId} từ ${currentPriority} sang ${nextPriority}`);
            ...
            <div className="flex flex-wrap gap-1 sm:gap-2 lg:gap-3">
              {taskViewButtons.map((btn) => (
                ...
                >
                  ...
                  <span className="whitespace-nowrap text-xs sm:text-sm lg:text-base">{btn.label}</span>
                </button>
              ))}
            </div>
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center flex-1 min-w-0">
                  <div className={`w-6 h-6 rounded-lg ${typeColors[typeMapping[task.type]?.code || 'KC']} flex items-center justify-center mr-2 flex-shrink-0`}>
                    {React.createElement(typeMapping[task.type]?.icon || Circle, { className: "w-3 h-3" })}
                  </div>
              ...
              {tasks.map((task) => (
                ...
                >
                  <td className="px-3 sm:px-4 lg:px-6 py-3 sm:py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-lg ${typeColors[typeMapping[task.type]?.code || 'KC']} flex items-center justify-center mr-2 sm:mr-3`}>
                        {React.createElement(typeMapping[task.type]?.icon || Circle, { className: "w-3 h-3 sm:w-4 sm:h-4" })}
                      </div>
