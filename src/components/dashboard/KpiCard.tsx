...
const KpiCard = ({ title, value, change, data, className, oldValue }: KpiCardProps) => {
  ...
  return (
    <Card className={`bg-white/95 backdrop-blur-lg border border-white/30 shadow-xl overflow-hidden rounded-[20px] transition-all ${className}`}>
      <CardContent className="p-6">
        <div className="flex flex-col">
          ...
          <div className="flex items-center mt-2">
            <span className={`inline-flex items-center text-xs font-medium ${isPositive ? 'text-emerald-500' : 'text-rose-500'}`}>
              {isPositive ? 
