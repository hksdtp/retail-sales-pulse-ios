...
// Helper functions for task operations (Firebase removed)
...
// Firebase functions removed - using mock data and Google Sheets only
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      try {
        ...
        // COMMENTED OUT: Không xóa dữ liệu Firebase để test date filter
        ...
        // FORCE LOAD Firebase tasks với multiple attempts
        console.log('🔥 FORCE LOADING Firebase tasks...');

        // Attempt 1: Check configuration
        const firebaseConfigured = isFirebaseConfigured();
        console.log(`🔥 Firebase configured: ${firebaseConfigured}`);

        // Attempt 2: Force initialize if not configured
        if (!firebaseConfigured) {
          ...
          try {
            ...
            FirebaseService.initializeApp(defaultConfig);
            console.log('✅ Firebase force initialized');
          } catch (error) {
            console.error('❌ Force Firebase init failed:', error);
          }
        }

        // Attempt 3: Load tasks from Firebase
        try {
          ...
          console.log('🔥 Firebase tasks type:', typeof firebaseTasks);
          console.log('🔥 Firebase tasks length:', firebaseTasks?.length);

          if (firebaseTasks && Array.isArray(firebaseTasks) && firebaseTasks.length > 0) {
            ...
            console.log('📋 ⚠️ No tasks found in Firebase or invalid response');
            console.log('📋 ⚠️ Firebase response details:', {
              exists: !!firebaseTasks,
              isArray: Array.isArray(firebaseTasks),
              length: firebaseTasks?.length,
              type: typeof firebaseTasks
            });
          }
        } catch (error) {
        ...
        console.log('🔧 [DEBUG] Starting permission filtering...');
        console.log('🔧 [DEBUG] Current user:', currentUser);
        console.log('🔧 [DEBUG] Raw tasks count:', rawTasksData.length);
        console.log('🔧 [DEBUG] Users count:', users?.length || 0);
        console.log('🔧 [DEBUG] Teams count:', teams?.length || 0);
        console.log('🔧 [DEBUG] Condition check - currentUser:', !!currentUser);
        console.log('🔧 [DEBUG] Condition check - users:', !!users);
        console.log('🔧 [DEBUG] Condition check - teams:', !!teams);

        // FORCE CREATE TEST TASK for debugging
        if (currentUser?.name === 'Khổng Đức Mạnh' && rawTasksData.length === 0) {
          console.log('🔧 [FORCE DEBUG] Creating test task for Khổng Đức Mạnh');
          const testTask: Task = {
            id: 'test_task_debug_001',
            title: 'Test Task - Debug Mode',
          ...
          rawTasksData = [testTask];
          console.log('🔧 [FORCE DEBUG] Test task created:', testTask);
        }

        // TEMPORARY DEBUG: Show all tasks for Khổng Đức Mạnh
        if (currentUser?.name === 'Khổng Đức Mạnh') {
          console.log('🔧 [TEMP DEBUG] Showing ALL tasks for Khổng Đức Mạnh');
          filteredTasksForRole = rawTasksData;
        } else if (currentUser && users && teams) {
          console.log('🔧 [DEBUG] ✅ All conditions met, proceeding with filtering...');
          permissionLog(
          ...
          if (isAdmin(userId) || isDirector(userRole)) {
            ...
            if (isAdmin(userId)) {
              ...
              console.log('🔧 [ADMIN] Admin user - showing ALL tasks');
              filteredTasksForRole = rawTasksData;
            } else {
              // Director xem công việc trong phòng ban của mình
              console.log('🔧 [DIRECTOR] Director user - filtering by department');
              const directorDepartment = currentUser.department_type || currentUser.department;
              console.log('🔧 [DIRECTOR] Director department:', directorDepartment);
              console.log('🔧 [DIRECTOR] Available users:', users.length);
              console.log('🔧 [DIRECTOR] Available teams:', teams.length);

              filteredTasksForRole = rawTasksData.filter((task) => {
                console.log(`🔧 [DIRECTOR] Checking task ${task.id} (${task.title})`);

                // 1. Tasks assigned to users in the same department
                if (task.assignedTo) {
                  const assignedUser = users.find((u) => u.id === task.assignedTo);
                  if (assignedUser) {
                    const userDepartment = assignedUser.department_type || assignedUser.department;
                    const isInDepartment = userDepartment === directorDepartment;
                    console.log(`  - Assigned to ${assignedUser.name} (dept: ${userDepartment}), match: ${isInDepartment}`);
                    if (isInDepartment) return true;
                  }
                }

                // 2. Tasks created by users in the same department
                if (task.user_id) {
                  const creator = users.find((u) => u.id === task.user_id);
                  if (creator) {
                    const creatorDepartment = creator.department_type || creator.department;
                    const isInDepartment = creatorDepartment === directorDepartment;
                    console.log(`  - Created by ${creator.name} (dept: ${creatorDepartment}), match: ${isInDepartment}`);
                    if (isInDepartment) return true;
                  }
                }

                // 3. Tasks belonging to teams in the same department
                if (task.teamId) {
                  const taskTeam = teams.find((t) => t.id === task.teamId);
                  if (taskTeam) {
                    const teamDepartment = taskTeam.department_type || taskTeam.department;
                    const isInDepartment = teamDepartment === directorDepartment;
                    console.log(`  - Team ${taskTeam.name} (dept: ${teamDepartment}), match: ${isInDepartment}`);
                    if (isInDepartment) return true;
                  }
                }

                console.log(`  - Task ${task.id}: No department match`);
                return false;
              });

              console.log(`🔧 [DIRECTOR] Filtered ${filteredTasksForRole.length}/${rawTasksData.length} tasks for department ${directorDepartment}`);
            }
          }
          // Nếu là Trưởng nhóm
          ...
          if (!currentUser) {
            console.log('🔧 [DEBUG] No current user - showing no tasks');
            permissionLog(
            ...
            console.log('🔧 [DEBUG] Users or teams not loaded yet - showing no tasks until loaded');
            permissionLog(
              `Đang tải dữ liệu người dùng và nhóm - chờ hoàn tất`,
              LogLevel.BASIC,
            );
            filteredTasksForRole = [];
          } else {
            console.log('🔧 [DEBUG] Unknown condition - showing no tasks');
            filteredTasksForRole = [];
          }
        }
        ...
        console.log('🔧 [DEBUG] - Filtered tasks count:', filteredTasksForRole.length);
        console.log('🔧 [DEBUG] - Filtered tasks:', filteredTasksForRole.map(t => ({ id: t.id, title: t.title, date: t.date })));

        // Lưu dữ liệu đã lọc phân quyền vào localStorage
        saveFilteredTasks(filteredTasksForRole);
        console.log('🔧 [DEBUG] Saved filtered tasks to localStorage');

        // Cập nhật state với dữ liệu đã lọc
        setTasks(filteredTasksForRole);
        console.log('🔧 [DEBUG] Updated tasks state');
        console.log('🔧 [DEBUG] ===== FINAL STATE DEBUG =====');
        console.log('🔧 [DEBUG] Tasks state will be set to:', filteredTasksForRole);
        console.log('🔧 [DEBUG] Tasks count:', filteredTasksForRole.length);
        console.log('🔧 [DEBUG] Sample tasks:', filteredTasksForRole.slice(0, 3).map(t => ({
          id: t.id,
          title: t.title,
          date: t.date,
          status: t.status,
