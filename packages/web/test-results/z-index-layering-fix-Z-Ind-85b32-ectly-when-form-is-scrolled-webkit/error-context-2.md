...
- Name: Z-Index Layering Fix - TaskFormDialog Dropdowns >> should work correctly when form is scrolled
- Location: /Users/nih/Bán lẻ/retail-sales-pulse-ios/packages/web/tests/z-index-layering-fix.spec.ts:192:3
...
<launched> pid=51045
[pid=51045] <gracefully close start>
```
...
```ts
  ...
  109 |     
  110 |     // Click on date picker
  111 |     const dateButton = page.locator('button:has-text("Chọn ngày thực hiện")');
  112 |     await dateButton.click();
  113 |     
  114 |     // Wait for popover to appear
  115 |     await page.waitForSelector('text=Hôm nay', { timeout: 2000 });
  116 |     
  117 |     // Verify popover is visible
  118 |     const popover = page.locator('text=Hôm nay').locator('..');
  119 |     await expect(popover).toBeVisible();
  120 |     
  121 |     // Test that we can interact with quick options
